...
lag_insects_yr_function <- function(x=x){
	x %>% 
		...
		dplyr::mutate(
			...
			# lagged insects
			CAyr_t_1 		= c(NA, utils::head(CAyr_t, -1)),
			MEyr_t_1 		= c(NA, utils::head(MEyr_t, -1)),
			CHyr_t_1 		= c(NA, utils::head(CHyr_t, -1)),
			DAyr_t_1 		= c(NA, utils::head(DAyr_t, -1)),
			Insectyr_t_1 		= c(NA, utils::head(Insectyr_t, -1)),
			NatInsectyr_t_1 	= c(NA, utils::head(NatInsectyr_t, -1))
		) %>%
		as.data.frame
}
